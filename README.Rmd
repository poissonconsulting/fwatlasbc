...
Note that blk and rm are from the original watershed while blue_line_key and downstream/upstream_route_measure are the equivalents for the stream segment in the network.
```{r}
network <- fwa_add_collection_to_watershed(wshed, "stream_network")
network
```

Convert the stream network into a table of the blue line keys with regularly spaced river meters starting at 0.
Note that blk is from the original watershed while blue_line_key and rm are for the river meter.
```{r}
