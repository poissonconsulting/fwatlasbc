...
Find gnis stream names using regular expression.
...
It is worth noting that there may be more than one stream with the same name.
```{r}
streams <- fwa_add_blks_to_stream_name(streams)
streams
```

Get river meters for the blue line keys.
```{r}
rm <- fwa_add_blks_to_stream_name(streams)
streams
```
