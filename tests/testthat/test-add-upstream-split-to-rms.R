test_that("fwa_add_upstream_split_to_rms simple example", {
  x <- data.frame(
    blk = 1L,
    rm = seq(1000L, 10000L, by = 1000L),
    parent_blk = NA_integer_,
    parent_rm = NA_integer_
  )
  ...
  expect_identical(
    colnames(x),
    c("blk", "rm", "parent_blk", "parent_rm", "new")
  )
...
test_that("fwa_add_upstream_split_to_rms simple example all", {
  ...
  x <- fwa_add_upstream_split_to_rms(
    x,
    data.frame(blk = 1, rm = 0, name = "new2")
  )

  expect_s3_class(x, "tbl")
  expect_identical(
    colnames(x),
    c("blk", "rm", "parent_blk", "parent_rm", "new2")
  )
...
test_that("fwa_add_upstream_split_to_rms simple example partway up main", {
  ...
  x <- fwa_add_upstream_split_to_rms(
    x,
    data.frame(blk = 1, rm = 1, name = "new2")
  )
...
test_that("fwa_add_upstream_split_to_rms on trib", {
  ...
  x <- fwa_add_upstream_split_to_rms(
    x,
    data.frame(blk = 2, rm = 1, name = "new2")
  )
...
test_that("fwa_add_upstream_split_to_rms off top", {
  ...
  x <- fwa_add_upstream_split_to_rms(
    x,
    data.frame(blk = 1, rm = 10, name = "new2")
  )
...
test_that("fwa_add_upstream_split_to_rms two splits", {
  ...
  expect_identical(
    colnames(x),
    c("blk", "rm", "parent_blk", "parent_rm", "new2", "new1")
  )
  expect_identical(nrow(x), 8L)
  expect_identical(x$new2, c(FALSE, rep(TRUE, 5), rep(FALSE, 2)))
