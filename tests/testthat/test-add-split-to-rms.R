...
test_that("fwa_add_split_to_rms simple example all", {
  x <- dplyr::tribble(
  ...
  x <- fwa_add_split_to_rms(x, data.frame(blk = 1, rm = 0, name = "new2"))
...
test_that("fwa_add_split_to_rms simple example partway up main", {
  x <- dplyr::tribble(
    ~blk, ~rm, ~parent_blk, ~parent_rm,
    1,     0,    NA_integer_,   NA_integer_,
    1,      1,    NA_integer_,   NA_integer_,
  ...
  x <- fwa_add_split_to_rms(x, data.frame(blk = 1, rm = 1, name = "new2"))
...
test_that("fwa_add_split_to_rms on trib", {
  x <- dplyr::tribble(
    ~blk, ~rm, ~parent_blk, ~parent_rm,
    1,     0,    NA_integer_,   NA_integer_,
    1,      1,    NA_integer_,   NA_integer_,
  ...
  x <- fwa_add_split_to_rms(x, data.frame(blk = 2, rm = 1, name = "new2"))
...
test_that("fwa_add_split_to_rms off top", {
  x <- dplyr::tribble(
    ~blk, ~rm, ~parent_blk, ~parent_rm,
    1,     0,    NA_integer_,   NA_integer_,
    1,      1,    NA_integer_,   NA_integer_,
  ...
  x <- fwa_add_split_to_rms(x, data.frame(blk = 1, rm = 10, name = "new2"))
...
test_that("fwa_add_split_to_rms two splits", {
  x <- dplyr::tribble(
    ~blk, ~rm, ~parent_blk, ~parent_rm,
    1,     0,    NA_integer_,   NA_integer_,
    1,      1,    NA_integer_,   NA_integer_,
