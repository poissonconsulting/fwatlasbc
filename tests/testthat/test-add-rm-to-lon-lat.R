...
test_that("fwa_add_rm_to_lon_lat works multiple ", {
  x <- fwa_add_rm_to_lon_lat(data.frame(lon = c(-132.26, -132.25), lat = 53.36), limit = 3)
  expect_s3_class(x, "sf")
  expect_identical(colnames(x), c("lon", "lat", "blk", "rm", "distance_to_lon_lat", "geometry"
  ))
...
test_that("fwa_add_rm_to_lon_lat works none ", {
  x <- fwa_add_rm_to_lon_lat(data.frame(lon = -132.26, lat = 53.36), tolerance = 500)
  expect_s3_class(x, "sf")
  expect_identical(colnames(x), c("lon", "lat", "blk", "rm", "distance_to_lon_lat", "geometry"
  ))
