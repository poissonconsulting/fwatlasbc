...
test_that("fwa_snap_rms_to_rms not snaps mouths to mouths", {
  ...
  expect_equal(
    rm$distance_to_new_rm,
    c(1803.47106759852, 0, 0, 0, 0, 1496.94367005726)
  )
  expect_s3_class(rm$geometry, "sfc_POINT")
})
...
test_that("fwa_snap_rms_to_rms can deal with mising blks", {
  ...
  expect_equal(
    x$distance_to_new_rm,
    c(3314.98934913696, 1837.8850050135, 772.26037010328, 0, 0)
  )
  expect_s3_class(x$geometry, "sfc_POINT")
  ...
  expect_equal(
    rm$distance_to_new_rm,
    c(NA, NA, 1837.8850050135, 0, 0, 1496.94367005726)
  )
  expect_s3_class(rm$geometry, "sfc_POINT")
})
...
test_that("fwa_snap_rms_to_rms multiple blks to 1 blk", {
  ...
  expect_equal(
    x$new_rm,
    c(0, 2000, 5000, 6000, 7000, 0, 2000, 5000, 6000, 7000)
  )
  ...
  expect_equal(
    rm$new_rm,
    c(NA, NA, NA, NA, NA, NA, 0L, 2000L, 5000L, 6000L, 7000L, 7000L)
  )
  expect_equal(
    rm$distance_to_new_rm,
    c(NA, NA, NA, NA, NA, NA, 0, 0, 0, 0, 0, 1496.94367005726)
  )
  expect_s3_class(rm$geometry, "sfc_POINT")
})
