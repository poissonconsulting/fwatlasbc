...
add_collection_to_watershed <- function(x, collection, filter, limit, offset,
  ...
  if(x$..fwa_intersect) {
    suppressWarnings(coll <- sf::st_intersection(coll, watershed))
    coll <- sf::st_make_valid(coll)
  }
  coll <- dplyr::mutate(coll, dplyr::across(dplyr::matches("gnis_id_\\d"), as.integer))
  if("geometry" %in% colnames(x)) {
    x <- x |> dplyr::select(-.data$geometry)
  }
  x |>
...
#' If the active sfc polygon column is called geometry replaced
#' by the geometry column of the column.
#'
