add_stream_network_to_blk <- function(x, epsg) {
  ...
  x <- x |>
    fwa_add_watershed_to_blk(rm = x$..fwa_rm2, epsg = epsg) |>
  ...
  x |>
    ...
    dplyr::filter(.data$..fwa_percent2 >= .data$..fwa_rm2 / max_rm)
}
...
fwa_add_stream_network_to_blk <- function(x, rm = 0,
  ...
  x |>
    ...
    dplyr::select(-.data$..fwa_id2, -.data$..fwa_rm2, -.data$..fwa_percent2)
}
